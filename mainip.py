...
class Console:
    ...
    def receive(self, message_id, text: str):
        ...
        if text == '/start':
            ...
            if self.subThread and self.subThread.is_alive():
                ...
                self.subThread = KThread(target=self.run,
                                         args=(text,), daemon=True)  # create thread for executing the command
                self.subThread.start()
                self.subThread.name = f'console {self.chatID}'
                self.manager.threads[self.subThread.ident] = self
    ...
    def run(self, script, startup=False):  # secondary thread
        ...
        del self.manager.threads[self.subThread.ident]
    ...
    def isRunning(self) -> bool:
        return self.subThread and self.subThread.is_alive()
...
class Terminator:
    ...
    def terminateCommand(self, hard=False):
        ...
        if console.isRunning():
            console.subThread.kill()
            # sleep(0.2)
            if console.isRunning():
                if not hard:
                    console.write(
                        "Unfortunately, I couldn't kill the command as it is a blocking call(such as time.sleep)."
        ...
        if hard:
            '''when the call is from the manager, I need to use looseSend because this console may have already
            been removed from manager.data . 
            This may happen non-deterministically, as the removal is performed by the main thread, while the message
            is sent by the wait thread'''
            looseSend(console.chatID, 'terminator called')
        else:
            console.write('terminator called')
...
class ConsolesManager:
    ...
    def remove(self, console: Console, hard=False):
        console.__exit__()
    ...
    def poll(self):
        while self.data:
            ...
            if updates:  # TODO: process more message at once (in this way some messages may be lost)
...
Token = '1268314054:AAHFugw_rOq1LbOUrrwDA23EGCp4tr4f4jI'
bot = telegram.Bot(token=Token)

path = 'https://api.telegram.org/bot%s' % Token
chatId = myChatID = 234884132
bold = '<b>%s</b>'
...
def debug_start():
    try:
        ...
        global manager
        manager = ConsolesManager().__enter__()
...
'''
    ...
    def reset():
        try:
            a = requests.get('%s/getUpdates' % path, params={'offset': 0}).json()['result'][-1]['update_id'] + 1
        except:
            a = 0
        return requests.get('%s/getUpdates' % path, params={'offset': a}).json()['result']

    def attach():
        hook = 'https://lm857.user.srcf.net/pythonConsole_bot/root.php'
        requests.get('%s/setWebhook' % path, params={'url': hook})
    ...
    def getUpdates():
        return requests.get('%s/getUpdates' % path, params={'offset': 0}).json()['result']     
'''
